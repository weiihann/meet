...
class TestParseEnv:
    def test_reads_simple_assignments(self):
        assert parse_env("MEET_NOTES=/tmp/vault\n") == {"MEET_NOTES": "/tmp/vault"}
...
class TestApplyEnv:
    ...
    def test_never_overrides_the_real_environment(self):
        """`MEET_NOTES=/override meet ...` must beat the .env file."""
        environ = {"MEET_NOTES": "/override"}
        apply_env({"MEET_NOTES": "/from-dotenv"}, environ)
        assert environ["MEET_NOTES"] == "/override"
