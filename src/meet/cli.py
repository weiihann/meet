...
from meet import capture, setup
...
def _transcribe_target(target: Path, args: argparse.Namespace) -> list[Segment]:
    ...
    engine = load_engine(
        args.engine,
        context=context_prompt(load_terms(args.glossary)),
...
def _summarise_to_notes(source: Path, args: argparse.Namespace) -> Path:
    """Summarise a transcript into a note in the notes directory."""
    transcript = source.read_text(encoding="utf-8")
    _say(f"summarising with {args.summariser}...")
    body = summarise(
        transcript,
        ", ".join(load_terms(args.glossary)),
...
def _process(target: Path, args: argparse.Namespace) -> None:
    """Transcribe then summarise, the whole way from audio to finished note."""
    transcript = _write_transcript(target, _transcribe_target(target, args))
    _summarise_to_notes(transcript, args)
...
def cmd_summarize(args: argparse.Namespace) -> int:
    ...
    _summarise_to_notes(source, args)
    return 0
...
def cmd_setup(args: argparse.Namespace) -> int:
    return setup.run(
        _say,
        with_sensevoice=args.with_sensevoice,
...
def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    print(f"  [{'ok' if dotenv.exists() else 'none'}] config: {dotenv}")
    checks = [
    ...
    for label, path in checks:
        ok = path.exists()
        problems += not ok
        hint = "" if ok else "  <- run `meet setup`"
        print(f"  [{'ok' if ok else 'MISSING'}] {label}: {path}{hint}")
    print(f"  glossary terms: {len(load_terms())}")
    # Notes are informational: the directory is created on demand, so its absence
    # is not a problem to report.
    print(f"  notes: {NOTES}")
    print(f"  language: {DEFAULT_LANGUAGE}")
    print(f"  recogniser: {ASR_MODEL_ID}")
    print(f"  summariser: {SUMMARY_MODEL_ID}")
    try:
...
def _add_engine_flag(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=f"language to pin the recogniser to (default {DEFAULT_LANGUAGE}, "
        "from MEET_LANGUAGE); zh also handles English embedded in Mandarin; "
        "auto detects per chunk but drifts to unrelated languages",
    )
...
def _add_glossary_flag(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--glossary",
        type=Path,
        default=None,
        help=f"vocabulary file to use for this run (default {GLOSSARY_FILE})",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_glossary_flag(transcribe)
    transcribe.set_defaults(func=cmd_transcribe)

    summarize = sub.add_parser("summarize", help="turn a transcript into a note")
    summarize.add_argument("transcript")
    _add_note_flags(summarize)
    _add_glossary_flag(summarize)
    summarize.set_defaults(func=cmd_summarize)

    setup_cmd = sub.add_parser("setup", help="download models and build the capture binary")
    setup_cmd.add_argument(
        "--with-sensevoice",
        action="store_true",
        help="also fetch the optional SenseVoice recogniser (929 MB)",
    )
    setup_cmd.add_argument(
        "--no-prefetch",
        dest="prefetch",
        action="store_false",
        help="do not download the recognising and summarising models "
        "(several GB); let them download on first use instead",
    )
    setup_cmd.add_argument(
        "--no-launcher",
        dest="launcher",
        action="store_false",
        help="do not install a `meet` command on PATH; use `uv run meet` instead",
    )
