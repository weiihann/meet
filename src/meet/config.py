...
1. the real environment, so `MEET_VAULT=/tmp meet ...` always wins
...
#: Where finished notes are written. There is no sensible universal default, so
#: `meet doctor` reports when this does not exist rather than silently creating it.
VAULT = Path(os.environ.get("MEET_VAULT", Path.home() / "Documents/Obsidian")).expanduser()
NOTES_SUBDIR = os.environ.get("MEET_NOTES_SUBDIR", "Notes")
