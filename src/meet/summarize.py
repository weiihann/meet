"""Summarising a transcript into a Markdown note.
...
Notes are Markdown with YAML frontmatter (`created`, `org`, tags), which Obsidian
and most other note tools read directly.
"""
...
from meet.config import NOTES, SUMMARY_MODEL_ID
...
#: Filename date format. Compact dates sort correctly and read cleanly in a
#: file listing; the `created` frontmatter field stays ISO.
FILENAME_DATE = "%Y%m%d"


def note_path(when: date, title: str) -> str:
    """Where the finished note belongs."""
    stamp = when.strftime(FILENAME_DATE)
    return str(NOTES / f"{stamp} Meeting - {safe_title(title)}.md")


def render_note(when: date, title: str, body: str, transcript: str, org: str = "") -> str:
    """Wrap the model's summary in the meeting-note format."""
    return (
        "---\n"
